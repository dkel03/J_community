...
router.post('/:_suggestion', function(req, res, next) {
  ...
  Suggestion.findById(_suggestion)
    ...
    .then(r=> {
      if (!r) throw new Error('건의사항이 생성되지 않았습니다.')
...
router.put('/:id', (req, res, next) => {
  ...
  Comment.findOne({ _id })
    .then((r) => {
      if (!r) return next(createError(400, '댓글이 없습니다'))
      if (r._user._id.toString() !== req.user._id) {
        return next(createError(403, '본인이 작성한 댓글이 아닙니다.'))
      }
      return Comment.updateOne({ _id }, { $set: req.body })
    })
    .then(r => {
